...
function checkMobile(){
  if (!empty($_GET['m'])) {return true;}
  if (!empty($_GET['pc'])) {return false;}
...
<html>
  ...
  <body>
    <header onclick="link('/');" class="center">
      <h1 class="title">REMIND<br><div class="second_tile">Support</div></h1>
    </header>
    <nav class="center">
      <?php if (checkMobile()){ ?>
      <!--mobile nav-->
      <div class="menu" onclick="link('?request=ios');">
        <img class="menu" src="../../img/main/m_bw_main.jpg">
        <div class="text">
          <b class="title">iOS 웹앱</b>
          iOS 웹앱 설치 방법에 대해 알아봅니다.
        </div>
      </div>
      <?php }else{ ?>
      <!--pc nav-->
        <div class="polaroid" onclick="link('?request=ios');">
        <img src="../../img/bw_main.jpg">
        <div class="container">
          <p><h2 style="font-size: 150% !important;">iOS 웹앱</h2><br></p>
        </div>
        </div>
      <?php } ?>
    </nav>
    <article class="center">
      ...
      <?php } else {?>
        <div id="back" onclick="link('?1=1');" >&lt; back</div>
      <?php } ?>
    </article>
